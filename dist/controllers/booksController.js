"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteBook = exports.updateBook = exports.createBook = exports.getBookById = exports.getAllBooks = void 0;
var booksModel_1 = require("../models/booksModel");
var authorsModel_1 = require("../models/authorsModel");
var getAllBooks = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var books, e_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, booksModel_1.BookModel.find()];
            case 1:
                books = _a.sent();
                res.status(200).json(books);
                return [3 /*break*/, 3];
            case 2:
                e_1 = _a.sent();
                res.status(500).json({ message: 'Error getting Books' });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.getAllBooks = getAllBooks;
var getBookById = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var book, e_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, booksModel_1.BookModel.findById(req.params.id)];
            case 1:
                book = _a.sent();
                res.status(200).json(book);
                return [3 /*break*/, 3];
            case 2:
                e_2 = _a.sent();
                res.status(500).json({ message: 'Error getting Book' });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.getBookById = getBookById;
var createBook = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var book, author, e_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 5, , 6]);
                return [4 /*yield*/, booksModel_1.BookModel.create(req.body)];
            case 1:
                book = _a.sent();
                return [4 /*yield*/, authorsModel_1.AuthorsModel.findById(req.body.authorId)];
            case 2:
                author = _a.sent();
                if (!author) return [3 /*break*/, 4];
                author.books.push(book._id);
                return [4 /*yield*/, author.save()];
            case 3:
                _a.sent();
                _a.label = 4;
            case 4:
                res.status(200).json(book);
                return [3 /*break*/, 6];
            case 5:
                e_3 = _a.sent();
                res.status(500).json({ message: 'Error creating Book' });
                return [3 /*break*/, 6];
            case 6: return [2 /*return*/];
        }
    });
}); };
exports.createBook = createBook;
var updateBook = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var book, e_4;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, booksModel_1.BookModel.findByIdAndUpdate({ _id: req.params.id }, req.body, { new: true, })];
            case 1:
                book = _a.sent();
                if (!book) {
                    res.status(400).json({ message: 'Book not found' });
                }
                res.status(200).json(book);
                return [3 /*break*/, 3];
            case 2:
                e_4 = _a.sent();
                res.status(500).json({ message: 'Error updating Book' });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.updateBook = updateBook;
var deleteBook = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var book_1, author, e_5;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 7, , 8]);
                return [4 /*yield*/, booksModel_1.BookModel.findById(req.params.id)];
            case 1:
                book_1 = _a.sent();
                if (!book_1) {
                    res.status(400).json({ message: 'Book not found' });
                }
                if (!book_1) return [3 /*break*/, 6];
                return [4 /*yield*/, authorsModel_1.AuthorsModel.findById(book_1.authorId)];
            case 2:
                author = _a.sent();
                if (!author) return [3 /*break*/, 4];
                author.books = author.books.filter(function (bookId) { return bookId !== book_1._id; });
                return [4 /*yield*/, author.save()];
            case 3:
                _a.sent();
                _a.label = 4;
            case 4: return [4 /*yield*/, book_1.remove()];
            case 5:
                _a.sent();
                _a.label = 6;
            case 6:
                res.status(200).json({ message: "Deleted ".concat(req.params.id) });
                return [3 /*break*/, 8];
            case 7:
                e_5 = _a.sent();
                res.status(500).json({ message: 'Error deleting Book' });
                return [3 /*break*/, 8];
            case 8: return [2 /*return*/];
        }
    });
}); };
exports.deleteBook = deleteBook;
